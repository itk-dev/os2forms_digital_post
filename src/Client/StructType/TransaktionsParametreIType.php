...
class TransaktionsParametreIType extends AbstractStructBase
{
     ...
     * @param string $transaktionsDatoTid
     * @return \Drupal\os2forms_digital_post\Client\StructType\TransaktionsParametreIType
     */
     ...
     * @uses \Drupal\os2forms_digital_post\Client\EnumType\KvitteringsTypeKodeType::valueIsValid()
     * @uses \Drupal\os2forms_digital_post\Client\EnumType\KvitteringsTypeKodeType::getValidValues()
    ...
    public function setKvitteringsTypeKode(?string $kvitteringsTypeKode = null): self
    {
        ...
        if (!\Drupal\os2forms_digital_post\Client\EnumType\KvitteringsTypeKodeType::valueIsValid($kvitteringsTypeKode)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Drupal\os2forms_digital_post\Client\EnumType\KvitteringsTypeKodeType', is_array($kvitteringsTypeKode) ? implode(', ', $kvitteringsTypeKode) : var_export($kvitteringsTypeKode, true), implode(', ', \Drupal\os2forms_digital_post\Client\EnumType\KvitteringsTypeKodeType::getValidValues())), __LINE__);
        }
