...
class PostParametreType extends AbstractStructBase
{
     ...
     * @uses \Drupal\os2forms_digital_post\Client\EnumType\PostKategoriKodeType::valueIsValid()
     * @uses \Drupal\os2forms_digital_post\Client\EnumType\PostKategoriKodeType::getValidValues()
     * @throws InvalidArgumentException
     * @param string $postKategoriKode
     * @return \Drupal\os2forms_digital_post\Client\StructType\PostParametreType
     */
    public function setPostKategoriKode(?string $postKategoriKode = null): self
    {
        // validation for constraint: enumeration
        if (!\Drupal\os2forms_digital_post\Client\EnumType\PostKategoriKodeType::valueIsValid($postKategoriKode)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Drupal\os2forms_digital_post\Client\EnumType\PostKategoriKodeType', is_array($postKategoriKode) ? implode(', ', $postKategoriKode) : var_export($postKategoriKode, true), implode(', ', \Drupal\os2forms_digital_post\Client\EnumType\PostKategoriKodeType::getValidValues())), __LINE__);
        }
